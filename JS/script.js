function calcularNomina() {
  ...
  const diasSemestre = parseInt(document.getElementById('diastrabajadossemestre').value);
  ...
  const primaServicios = (salarioMensual * diasSemestre) / 360;
  const vacaciones = (salarioMensual * diasTrabajados) / 720;
  const salarioTotalGlobal = cesantias + interesesCesantias + primaServicios + vacaciones;
